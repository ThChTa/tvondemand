 ...
 * @author PX
 */
public class Employee_catalogController implements Initializable {
    ...
    ObservableList<String> oblist12 = FXCollections.observableArrayList();
    ...
    private void film_fetch(ActionEvent event) {
        ...
        try{    
         conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/tvondemand","root","root");
         ResultSet rs1 = conn.createStatement().executeQuery("SELECT film_id FROM inventory");
          
         while(rs1.next()){
             oblist12.add(rs1.getString("film_id"));
         }
         }
        catch(Exception e){
            JOptionPane.showMessageDialog(null,e);
        ...
        try{    
         ...
         while(rs.next()){
             if(oblist12.contains(rs.getString("film_id"))){ //Elegxos gia iparksi tou film_is ston inventory
                 oblist.add(new Model_Film_Table(rs.getString("film_id"),rs.getString("title")+"(inv)"));       //Onomata metavliton apo vasi
                }
             else{
    ...
    private void fetch_series(ActionEvent event) {
        ...
        try{    
         conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/tvondemand","root","root");
         ResultSet rs1 = conn.createStatement().executeQuery("SELECT series_id FROM series_inventory");
          
         while(rs1.next()){
             oblist12.add(rs1.getString("series_id"));
         }
         }
        catch(Exception e){
            JOptionPane.showMessageDialog(null,e);
        ...
        try{    
         ...
         while(rs.next()){
             if(oblist12.contains(rs.getString("series_id"))){ //Elegxos gia iparksi tou series_id sto series_inventory
                  oblist4.add(new Model_Series_Table(rs.getString("series_id"),rs.getString("title")+"(inv)"));       //Onomata metavliton apo vasi
                }
             else{
