 ...
 * @author PX
 */
public class Admin_employeesController implements Initializable {
    ...
    @FXML
    private TableView<Model_Employee_Table> epmloyees_table;
    @FXML
    private TableColumn<Model_Employee_Table,String> col_id;
    @FXML
    private TableColumn<Model_Employee_Table, String> col_fname;
    @FXML
    private TableColumn<Model_Employee_Table, String> col_lname;
    @FXML
    private TableColumn<Model_Employee_Table, String> col_mail;
    @FXML
    private TableColumn<Model_Employee_Table, String> col_address;
    @FXML
    private TableColumn<Model_Employee_Table, String> col_district;
    @FXML
    private TableColumn<Model_Employee_Table, String> col_pcode;
    @FXML
    private TableColumn<Model_Employee_Table, String> col_phone;
    @FXML
    private TableColumn<Model_Employee_Table, String> col_city;
    @FXML
    private TableColumn<Model_Employee_Table, String> col_country; 
    @FXML
    private TableColumn<Model_Employee_Table, String> col_gender;
    @FXML
    private TableColumn<Model_Employee_Table, String> col_date_of_hire;
    @FXML
    private TableColumn<Model_Employee_Table, String> col_salary;
    @FXML
    private TableColumn<Model_Employee_Table, String> col_rating;
    ...
    ObservableList<Model_Employee_Table> oblist = FXCollections.observableArrayList();
    ...
    private ObservableList<String> rating_list = FXCollections.observableArrayList("A1","A2","A3","B1","B2","B3","C1","C2","C3");
    ...
    private void fetch(ActionEvent event) {
        ...
        try{    
         conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/tvondemand","root","root");
         ResultSet rs = conn.createStatement().executeQuery("SELECT employee.employee_id, employee.first_name, employee.last_name,employee.email,employee.gender,employee.date_of_hire"
         ...
         while(rs.next()){
             oblist.add(new Model_Employee_Table(rs.getString("employee_id"),rs.getString("first_name"),rs.getString("last_name"),
                     rs.getString("email"),rs.getString("gender"),rs.getString("date_of_hire"),rs.getString("salary"),
                     rs.getString("rating"),rs.getString("address"),rs.getString("district"),rs.getString("postal_code"),rs.getString("phone"),
        ...
        col_date_of_hire.setCellValueFactory(new PropertyValueFactory<>("date_of_hire"));
        ...
        epmloyees_table.setItems(oblist);
        
       
        // Wrap the ObservableList in a FilteredList (initially display all data).
        FilteredList<Model_Employee_Table> filteredData = new FilteredList<>(oblist, b -> true);    //SearchBar
		...
		filterField.textProperty().addListener((observable, oldValue, newValue) -> {
			filteredData.setPredicate(employee -> {
				...
				if (employee.getEmployee_id().toLowerCase().indexOf(lowerCaseFilter) != -1) {
					return true; // Filter matches last name.
				}
                                else if (employee.getFname().toLowerCase().indexOf(lowerCaseFilter) != -1) {
					return true; // Filter matches last name.
				}
				     ...
				     return true;
                                ...
                                else if (employee.getDate_of_hire().toLowerCase().indexOf(lowerCaseFilter) != -1){
				     return true;
                                }
                                else if (employee.getSalary().toLowerCase().indexOf(lowerCaseFilter) != -1){
				     return true;
                                }
                                else if (employee.getRating().toLowerCase().indexOf(lowerCaseFilter) != -1){
				     return true;
                                }
                                else if (employee.getAddress().toLowerCase().indexOf(lowerCaseFilter) != -1){
				     return true;
                                }
                                else if (employee.getDistrict().toLowerCase().indexOf(lowerCaseFilter) != -1){
				     return true;
                                }
                                else if (employee.getPcode().toLowerCase().indexOf(lowerCaseFilter) != -1){
				     return true;
                                }
                                else if (employee.getPhone().toLowerCase().indexOf(lowerCaseFilter) != -1){
				     return true;
                                }
                                else if (employee.getCity().toLowerCase().indexOf(lowerCaseFilter) != -1){
				     return true;
                                }
                                else if (employee.getCountry().toLowerCase().indexOf(lowerCaseFilter) != -1){
				     return true;
                                }
				     else  
		...
		SortedList<Model_Employee_Table> sortedData = new SortedList<>(filteredData);
		
		// 4. Bind the SortedList comparator to the TableView comparator.
		// 	  Otherwise, sorting the TableView would have no effect.
		sortedData.comparatorProperty().bind(epmloyees_table.comparatorProperty());
		
		// 5. Add sorted (and filtered) data to the table.
		epmloyees_table.setItems(sortedData);
            ...
            Logger.getLogger(Admin_employeesController.class.getName()).log(Level.SEVERE, null, ex);
            }
    ...
    private void cust_submit1(ActionEvent event) {
        ...
        if(email.getText().trim().isEmpty()||fname.getText().trim().isEmpty()||lname.getText().trim().isEmpty()||salary.getText().trim().isEmpty()
                ||rating.getValue()==null||gender.getValue()==null){
        ...
        String password="root";
         ...
         stmt=conn.prepareStatement(query);
            ...
            stmt.setString(8, salary.getText());
            stmt.setString(9, rating.getValue());
    ...
    private void show_stoixeia(MouseEvent event) {
        index = epmloyees_table.getSelectionModel().getSelectedIndex();
       if(index<=-1){
           update_button.setDisable(true);
           delete_button.setDisable(true);
           make_admin_button.setDisable(true);
           return;
       }
       else if(index>-1){
       ...
       make_admin_button.setDisable(false);
    ...
    private void delete(ActionEvent event) {
        try{    
         ...
         String query ="DELETE FROM employee WHERE address_id = '"+update_address_id+"'";
          stmt=conn.prepareStatement(query);
        ...
        String up_fname,up_lname,up_address,up_district,up_pcode,up_phone,up_salary;
         ...
         while(rs.next()){
            ...
            JOptionPane.showMessageDialog(null,e);
             ...
             try{    
         conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/tvondemand","root","root");
         String query ="UPDATE employee SET first_name = '"+up_fname+"' ,last_name = '"+up_lname+"' ,salary='"+up_salary+"' WHERE address_id = '"+update_address_id+"'";
          stmt=conn.prepareStatement(query);
          stmt.executeUpdate(query);
    ...
    @FXML
    private void make_admin(ActionEvent event) {
        index = epmloyees_table.getSelectionModel().getSelectedIndex();
        ...
        String password="root";
         ...
         stmt=conn.prepareStatement(query);
            ...
            stmt.setString(6, ind_address_id);
            stmt.setString(7, col_salary.getCellData(index).toString());
