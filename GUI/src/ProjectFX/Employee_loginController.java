...
public class Employee_loginController implements Initializable {
    ...
    @FXML
    private AnchorPane employee_login;
    @FXML
    ...
    @FXML
    private void to_employee_login(ActionEvent event) throws IOException {
         int index;
        oblist.clear();
        ...
        try{
            ...
            ResultSet rs = conn.createStatement().executeQuery("SELECT email FROM employee");
        ...
        if(index!=-1 && c1.equals(c2)){
            try{
                conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/tvondemand","root","root");
                ResultSet rs = conn.createStatement().executeQuery("SELECT employee_id,first_name,last_name,address_id,gender,date_of_hire,salary,rating FROM employee "
                ...
                while(rs.next()){
                    ...
                    hire_date = rs.getString("date_of_hire");
            ...
            try{
                conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/tvondemand","root","root");
                String query = "UPDATE current_employee SET employee_id ='"+id+"',first_name='"+fname+"',last_name='"+lname+"'"
                    + ",email = '"+cust_username.getText()+"',date_of_hire = '"+hire_date+"',gender = '"+gender+"',country ='"+country+"',address ='"+address+"'"
                    + ",district ='"+district+"',city = '"+city+"',postal_code = '"+p_code+"',phone = '"+phone+"',salary ='"+salary+"',rating ='"+rating+"',in_use = 1";
        ...
        Parent root1 = FXMLLoader.load(getClass().getResource("employee_page.fxml"));
        stage = (Stage)((Node)event.getSource()).getScene().getWindow();
        scene = new Scene(root1);
        stage.setScene(scene);
        scene.setFill(Color.TRANSPARENT);
