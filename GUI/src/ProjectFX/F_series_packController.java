 ...
 * @author PX
 */
public class F_series_packController implements Initializable {
    ...
    private void rental(ActionEvent event) {
       ...
       try{    
         conn=DriverManager.getConnection("jdbc:mysql://localhost:3306/tvondemand","root","root");
         ResultSet rs2 = conn.createStatement().executeQuery("SELECT series_full_pack_price FROM pricing");//lipsi tis timis ana epeisodeio seiras
         
         while(rs2.next()){
            ep_price = rs2.getDouble("series_full_pack_price");
         }
        }
        catch(Exception e){
            JOptionPane.showMessageDialog(null,e);
